...
- **Merging several cues into one prose field? Rewrite it and restore punctuation.** When consecutive cues are condensed into one flowing text field (\`quoteText\`, \`bodyText\`, \`contentText\`, \`descText\`, \`footText\`), summarise and rewrite them into fluent prose — never concatenate the cue texts verbatim. SRT captions have punctuation stripped; when you merge lines, restore the missing commas (，) at natural pauses so the merged text reads as one smooth passage. Joining cues with bare spaces looks broken on screen. (Single-line heading fields stay ultra-short with no punctuation at all — see above.)
- **Mark emphasis with {{...}}.** Every display text field accepts inline emphasis markers: wrap the single most important word or short phrase in double curly braces (e.g. \`靠的就是{{AI}}\`, \`现在{{公开}}\`), and it renders in the component's emphasis colour (重点文字颜色, a per-component style prop the user tunes in the panel — you cannot set the colour, only the markers). Use at most 1-2 marks per field, on the true takeaway of the main heading first; never wrap a whole sentence, never nest.
