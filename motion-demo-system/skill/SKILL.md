...
| t3-06 | 横向对比双列卡片组(正反对照) |  按「组」依次入场；组内左右两卡可分别设定出现时间（右卡 atB 留空时与左卡同步点亮），卡片上浮淡入，要点逐条上浮淡入跟进（间距由 staggerMs 控制），可跟着口播左右分开起落。 | Two-sided comparison such as old vs new or plan A vs plan B, with 1-2 comparison groups and 2-3 bullet points per side. | Avoid single-sided lists, metrics without contrast, timelines, and more than 2 groups. | [reference](references/components/t3-06.md) |
