// scripts/standalone-validate-entry.mjs
...
var MAX_TRACK_INDEX = 63;
var MAX_TRACK_COUNT = MAX_TRACK_INDEX + 1;
var MAX_EFFECT_INSTANCES = 256;
var MAX_ALLOCATION_SEARCH_STEPS = 5e4;

// src/project/schema.ts
var positiveNumber = z.number().finite().positive();
var positiveInteger = z.number().int().positive();
var nonnegativeInteger = z.number().int().nonnegative();
var PlacementPresetSchema = z.enum([
  "auto",
  "left-top",
  "left-center",
  "left-bottom",
...
var SubtitleCueSchema = z.object({
...
var AgentInputVideoSchema = z.object({
  width: positiveNumber,
  height: positiveNumber,
  fps: positiveNumber,
  durationMs: positiveNumber
}).strict();
var MotionProjectVideoSchema = z.object({
  width: positiveNumber,
  height: positiveNumber,
  fps: positiveNumber,
  durationInFrames: positiveInteger
}).strict();
var AgentDraftComponentSchema = z.object({
...
var AgentDraftSceneSchema = z.object({
  sceneId: z.string(),
...
var MotionEffectInstanceSchema = z.object({
  instanceId: z.string(),
...
var MotionProjectSchema = z.object({
...
// src/project/cueTiming.ts
function cueFrameInterval(cue, fps, projectDuration) {
  const duration = Math.max(1, Math.round(projectDuration));
  const safeFps = Math.max(Number.EPSILON, fps);
  const rawStart = Math.ceil(cue.startMs * safeFps / 1e3);
  const rawEnd = Math.ceil(cue.endMs * safeFps / 1e3);
...
// src/project/trackAllocation.ts
var overlaps = (left, right) => left.startFrame < right.endFrame && right.startFrame < left.endFrame;
function exceedsMaximumConcurrency(fixed, pending) {
  const events = [
  ...
  const fixedCounts = /* @__PURE__ */ new Map();
  let activePending = 0;
...
function allocateTimelineTracks(fixed, pending, options = {}) {
  ...
  const conflicts = pending.map(() => /* @__PURE__ */ new Set());
  ...
  const baseCandidates = pending.map((interval) => {
    const futureReservation = (track) => fixed.reduce((next, reservation) => reservation.track === track && reservation.startFrame >= interval.endFrame ? Math.min(next, reservation.startFrame) : next, Number.POSITIVE_INFINITY);
    return Array.from({ length: MAX_TRACK_COUNT }, (_, track) => track).filter((track) => !fixed.some((reservation) => reservation.track === track && overlaps(interval, reservation))).sort((left, right) => futureReservation(left) - futureReservation(right) || left - right);
  });
  if (baseCandidates.some((candidates) => candidates.length === 0)) {
    return { ok: false, code: "track_capacity", reason: "constraints", searchSteps: 0 };
  }
  const requestedSearchStepLimit = options.searchStepLimit === void 0 || !Number.isFinite(options.searchStepLimit) ? MAX_ALLOCATION_SEARCH_STEPS : Math.max(0, Math.floor(options.searchStepLimit));
  const searchStepLimit = Math.min(MAX_ALLOCATION_SEARCH_STEPS, requestedSearchStepLimit);
  let searchSteps = 0;
  let searchLimitReached = false;
  const assignments = new Array(pending.length).fill(-1);
  const availableTracks = (index) => baseCandidates[index].filter((track) => ![...conflicts[index]].some((other) => assignments[other] === track));
  const assign = (remaining) => {
    ...
    for (let index = 0; index < pending.length; index += 1) {
      ...
      if (selected === -1 || candidates.length < selectedCandidates.length || candidates.length === selectedCandidates.length && (pending[index].key.localeCompare(pending[selected].key) < 0 || pending[index].key === pending[selected].key && index < selected)) {
        selected = index;
    ...
    for (const track of selectedCandidates) {
      ...
      const forwardValid = [...conflicts[selected]].every((other) => assignments[other] !== -1 || availableTracks(other).length > 0);
...
// src/project/validateDraftCore.ts
var warningLength = (prop) => {
  if (prop.semanticRole === "title" || prop.semanticRole === "label") return 40;
  if (prop.semanticRole === "body") return 80;
  return 120;
};
var contentMatchesType = (value, definition) => {
  if (definition.type === "number") {
    return typeof value === "number" && Number.isFinite(value) && (definition.min === void 0 || value >= definition.min) && (definition.max === void 0 || value <= definition.max);
  }
  if (definition.type === "list") return Array.isArray(value);
  return typeof value === "string";
};
var boxFor = (preset, definition, video) => {
  ...
  const horizontal = preset.startsWith("right") ? video.width - width : preset === "full-width" ? 0 : preset === "auto" && typeof defaultX === "number" ? defaultX : 0;
  const vertical = preset.endsWith("bottom") ? video.height - height : preset.endsWith("center") || preset === "full-width" ? (video.height - height) / 2 : preset === "auto" && typeof defaultY === "number" ? defaultY : 0;
  return { left: horizontal, top: vertical, right: horizontal + width, bottom: vertical + height };
};
var boxesOverlap = (left, right) => left.left < right.right && left.right > right.left && left.top < right.bottom && left.bottom > right.top;
function validateAgentDraftCore(draftValue, input, metaSource2, existingEffects = [], allocationOptions = {}) {
  const errors = [];
  const warnings = [];
  const parsed = AgentDraftSchema.safeParse(draftValue);
  if (!parsed.success) {
    for (const issue of parsed.error.issues) {
      const code = issue.path[0] === "schemaVersion" ? "schema_version" : issue.path[0] === "componentLibraryVersion" ? "library_version" : issue.code === "too_big" && issue.path[0] === "scenes" ? "effect_capacity" : "invalid_schema";
  ...
  const usedCueIds = /* @__PURE__ */ new Set();
  const placed = [];
  const pendingIntervals = [];
  const projectDuration = Math.max(1, Math.round(input.video.durationMs * input.video.fps / 1e3));
  draft.scenes.forEach((scene, sceneIndex) => {
    ...
    scene.sourceCueIds.forEach((cueId, cueIndex) => {
      ...
      if (!cueIds.has(cueId) || !cue) {
        errors.push({
          code: "unknown_cue",
          message: `Unknown source cue: ${cueId}`,
    ...
    referencedCues.slice(1).forEach((cue, index) => {
      ...
      if (cue.startMs < previous.startMs || cue.endMs < previous.endMs) {
        errors.push({
          code: "cue_order",
          message: "Source cues must form a monotonic scene interval.",
          path: ["scenes", sceneIndex, "sourceCueIds", index + 1]
        });
      }
    });
    const sceneInterval = firstCue && lastCue ? cueFrameInterval(
    ...
    if (firstCue && lastCue && !sceneInterval) {
      errors.push({
        code: "cue_bounds",
        message: "Source cues compile outside the video frame range.",
    ...
    if (scene.components.length > 2) {
      warnings.push({
        ...
        message: "A scene should have no more than two visual subjects.",
        path: ["scenes", sceneIndex, "components"]
      });
    }
    scene.components.forEach((component, componentIndex) => {
      const path = ["scenes", sceneIndex, "components", componentIndex];
      ...
      const definition = metaSource2.get(component.componentId);
      if (!definition) {
...
// scripts/standalone-validate-entry.mjs
var jsonPath = (parts = []) => `$${parts.map((part) => typeof part === "number" ? `[${part}]` : /^[A-Za-z_$][\w$]*$/.test(part) ? `.${part}` : `[${JSON.stringify(part)}]`).join("")}`;
var diagnostic = ({ code, message, path }) => ({
  code,
  message,
  path: jsonPath(path)
});
var fail = (errors, warnings = []) => {
